  ...
  <div class="card">
    <div class="card-body p-5">
      <div class="card-title d-flex align-items-center">
        ...
        <h5 class="mb-0 text-primary">Edit Pengajar</h5>
      </div>
      <hr>
      ...
      <form class="row g-3" action="{{ route('users.updatePengajar', $user->id) }}" method="POST">
        @csrf
        @method('PUT')
        ...
        <div class="col-md-12">
          ...
          <textarea id="address" name="address" class="form-control @error('address') is-invalid @enderror" rows="3">{{ old('address', $user->alamat) }}</textarea>
          @error('address') <div class="invalid-feedback">{{ $message }}</div> @enderror
        </div>

        <div class="col-12 d-flex gap-2">
          <button type="submit" class="btn btn-primary">Simpan</button>
          <a href="{{ route('users.pengajar') }}" class="btn btn-secondary">Batal</a>
        </div>
      </form>
    </div>
  </div>
